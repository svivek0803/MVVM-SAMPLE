...
extension ProfileModel: MappableProtocol{

    func mapToPersistenceObject() -> RealmProfile {
        let model = RealmProfile()
        model.id = id ?? 0
        model.name =  ""
        model.email = email ?? ""
